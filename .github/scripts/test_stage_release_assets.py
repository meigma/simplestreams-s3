...
class StageReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "simplestreams-s3_1.2.3_darwin_amd64",
                    "simplestreams-s3_1.2.3_darwin_amd64.sbom.json",
                    "simplestreams-s3_1.2.3_darwin_arm64",
                    "simplestreams-s3_1.2.3_darwin_arm64.sbom.json",
                    "simplestreams-s3_1.2.3_linux_amd64",
                    "simplestreams-s3_1.2.3_linux_amd64.sbom.json",
                    "simplestreams-s3_1.2.3_linux_arm64",
                    "simplestreams-s3_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/simplestreams-s3_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/simplestreams-s3_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="simplestreams-s3_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("simplestreams-s3_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("simplestreams-s3_1.2.3_linux_amd64", "0" * 64)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset simplestreams-s3_1.2.3_linux_arm64", stderr)
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"simplestreams-s3_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "simplestreams-s3_1.2.3_freebsd_amd64"
