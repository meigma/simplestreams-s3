...
## 2026-07-19 15:05 — V1 acceptance and operator surface
Completed the remaining Phase 5 implementation on `feat/phase5-telemetry-acceptance` in three reviewable commits: `7eb952d` adds transition-only readiness logs while keeping metrics aligned with cached staleness and drain state; `3559923` adds mandatory race and real-Incus CI gates; and `583ddb0` replaces template material with the V1 operator, configuration, observability, verification, and security references.

The Incus gate publishes the real split-VM fixture to disposable MinIO through production services, serves it over locally trusted HTTPS, then uses the actual Incus CLI to list the alias, import the VM, and assert the exact metadata-first fingerprint. I rehearsed the same CI procedure successfully inside a disposable Ubuntu 24.04 Lima guest with Incus 6.0.0 before publishing it.

Local evidence is green: `moon run root:check root:integration`, `go test -count=1 -race ./...`, and the tagged integration suite pass. GoReleaser 2.15.4 validated the configuration and built the four darwin/linux amd64/arm64 snapshot binaries with SBOMs and checksums. `mise run image-local` built the signed Wolfi apk and apko image; the loaded image reports UID 65532, contains no shell, and passes both `--version` and `version` smoke tests. The hosted PR checks and branch-targeted Release Dry Run remain to be recorded after push.

## 2026-07-19 15:17 — Phase 5 review gate
Opened PR #12, `feat: complete Phase 5 telemetry and V1 acceptance`, at final head `8442a5e1a9ba51a75333b14ce6f40d12ce2a47bd`. Hosted feedback produced two useful corrections before handoff: the Incus fixture now matches the runner architecture, and the security gate prompted OpenTelemetry v1.44.0 plus structured readiness JSON. A fresh official vulnerability scan then found the Go 1.26.4 standard library issue GO-2026-5856, so the repository now pins Go 1.26.5 with verified URLs and checksums for all four supported platforms. Final `govulncheck ./...` reports no vulnerabilities.

All exact-head acceptance evidence is green. CI run 29705750099 passed aggregate Moon/MinIO, race, and actual Incus listing/import/fingerprint jobs; Kusari Inspector, CodeQL, and docs deployment checks passed. Release Dry Run 29705753221 passed GoReleaser binary/SBOM/checksum validation, native amd64 and arm64 melange builds, and the apko non-root container smoke test. The PR is ready for human review. No merge, tag, release, or session closeout was performed.
